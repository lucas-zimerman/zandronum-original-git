...
public:
	VSMatrix() {}
	VSMatrix(int) {}  // Constructor for identity matrix
	void loadIdentity() {}
};
#endif

// FVector4 doesn't exist in zandronum - define a simple 4-component vector type
struct FVector4
{
	float X, Y, Z, W;
	...
	FVector4(const float v[4]) : X(v[0]), Y(v[1]), Z(v[2]), W(v[3]) {}
	
	// Allow brace initialization: FVector4{1, 2, 3, 4}
	FVector4(std::initializer_list<float> init)
	{
		auto it = init.begin();
		X = it != init.end() ? *it++ : 0;
		Y = it != init.end() ? *it++ : 0;
...
// Don't include hw_material.h - it defines a different FMaterial class
// We'll use Zandronum's FMaterial from gl/textures/gl_material.h instead
// #include "common/textures/hw_material.h"  // Commented out - conflicts with Zandronum's FMaterial
#include "common/textures/texmanip.h"
#include "version.h"
// i_interface.h may not exist in zandronum - check if sysCallbacks is needed
// #include "i_interface.h"  // Commented out - may not be needed
#include "r_data/renderstyle.h"

struct FColormap;
class IVertexBuffer;
class IIndexBuffer;
// Forward declare Zandronum's FMaterial (from gl/textures/gl_material.h)
class FMaterial;
// Forward declare FTexture
class FTexture;

// DefaultRenderStyle helper - compatibility
inline FRenderStyle DefaultRenderStyle()
{
	return LegacyRenderStyles[STYLE_Normal];
}

// STYLEALPHA_MAX constant
#ifndef STYLEALPHA_MAX
#define STYLEALPHA_MAX 4
#endif

// Texture mode constants - these are already defined in gl_interface.h
// Only define missing ones
#ifndef TM_NORMAL
#define TM_NORMAL 0  // Same as TM_MODULATE
#endif
#ifndef TM_CLAMPY
#define TM_CLAMPY 2  // For texture clamp
#endif
#ifndef TM_ALPHATEXTURE
#define TM_ALPHATEXTURE 5
#endif
#ifndef TM_STENCIL
#define TM_STENCIL 6
#endif
...
// Texture flags - compatibility
#ifndef TEXF_Brightmap
enum
{
	TEXF_Brightmap = 0x100,
	TEXF_Glowmap = 0x200,
	TEXF_ClampY = 0x400,
};
#endif
...
// ERenderEffect enum - EFF_NONE, EFF_FOGBOUNDARY, EFF_SPHEREMAP are already defined in gl/renderer/gl_renderstate.h
// Forward declare or use the existing enum values
#ifndef EFF_NONE
#define EFF_NONE 0
#define EFF_FOGBOUNDARY 1
#define EFF_SPHEREMAP 2
#endif
// Only define additional effects not in the old enum
enum ERenderEffect
{
	EFF_BURN = 3,  // Start after EFF_SPHEREMAP (which is 2)
	EFF_STENCIL = 4,
	EFF_DITHERTRANS = 5,
	MAX_EFFECTS = 6
};
...
// FStateVec4 is already defined in gl/renderer/gl_renderstate.h - forward declare it
struct FStateVec4;

// Forward declaration - zandronum's FMaterial
class FMaterial;

struct FMaterialState
{
	FMaterial *mMaterial = nullptr;  // zandronum's FMaterial from gl/textures/gl_material.h
	int mClampMode;
	int mTranslation;
	int mOverrideShader;
	bool mChanged;
...
// Base class for hardware renderer - renamed to avoid conflict with old FRenderState
...
private:
	void SetMaterial(FMaterial *mat, int clampmode, int translation, int overrideshader)
	{
		...
		// zandronum's FMaterial doesn't have GetLayerFlags() or GetDetailScale()
		// mTextureModeFlags = mat->GetLayerFlags();
		// auto scale = mat->GetDetailScale();
		// mStreamData.uDetailParms = { scale.X, scale.Y, 2, 0 };
		mStreamData.uDetailParms = { 1.0f, 1.0f, 2, 0 };  // Default detail scale
	}

public:
	// SetMaterial with FTexture - implementation in .cpp file to avoid include dependency
	void SetMaterial(FTexture* tex, int upscalemask, int scaleflags, int clampmode, int translation, int overrideshader);
	...
	{
		// Simplified for zandronum - will need to be expanded when buffer objects are available
