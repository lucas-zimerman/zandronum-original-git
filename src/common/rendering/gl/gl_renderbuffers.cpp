...
{
	// Use index-based iteration for TArray compatibility
	for (unsigned int i = 0; i < mEyeFBs.Size(); i++)
		DeleteFrameBuffer(mEyeFBs[i]);

	for (unsigned int i = 0; i < mEyeTextures.Size(); i++)
		DeleteTexture(mEyeTextures[i]);
...
void FGLRenderBuffers::Setup(int width, int height, int sceneWidth, int sceneHeight)
{
	...
	// gl_multisample CVAR not available in zandronum yet - use default
	int multisample = 1;
	int samples = clamp(multisample, 0, mMaxSamples);
	bool needsSceneTextures = false; // SSAO not available in zandronum yet
...
void FGLRenderBuffers::BlitSceneToTexture()
{
	...
	// RFL_INVALIDATE_BUFFER not available in zandronum yet
...
{
	// Shadow map creation - simplified for zandronum
	// This can be expanded later if shadow mapping is needed
	int shadowMapSize = 1024; // Default shadow map size
	if (mShadowMapTexture.handle != 0 && mCurrentShadowMapSize == shadowMapSize)
		return;
	...
	ClearShadowMap();
	mShadowMapTexture = Create2DTexture("ShadowMap", GL_R32F, shadowMapSize, shadowMapSize);
	...
	mCurrentShadowMapSize = shadowMapSize;
}

//==========================================================================
//
// Makes the scene frame buffer active (multisample, depth, stencil, etc.)
//
//==========================================================================
